...
func TestGoodFeaturesToTrackAVX2MatchesScalar(t *testing.T) {
	...
	original := cornerUseAVX2
	cornerUseAVX2 = false
	want, wantErr := goodFeaturesToTrack(context.Background(), gray, 100, 0.01, 8, 7)
	cornerUseAVX2 = original
	got, gotErr := goodFeaturesToTrack(context.Background(), gray, 100, 0.01, 8, 7)
...
func BenchmarkGoodFeaturesToTrackAMD64(b *testing.B) {
	...
	for _, benchmark := range []struct {
		...
		b.Run(benchmark.name, func(b *testing.B) {
			...
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				points, err := goodFeaturesToTrack(context.Background(), gray, 500, 0.01, 15, 7)
				if err != nil {
					b.Fatal(err)
				}
