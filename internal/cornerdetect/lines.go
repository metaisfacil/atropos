...
// PerimeterBackground describes the scanner surface estimated from image edges.
...
func BackgroundDistanceSilhouette(src *image.NRGBA) (*image.Gray, PerimeterBackground) {
	...
	for y := 0; y < h; y++ {
		...
		for x := 0; x < w; x++ {
			...
			dstRow[x] = raster.ClampByte((distance - background.BlackPoint) * 255 / rangeWidth)
		}
	}
...
func clampedImagePoint(x, y float64, w, h int) image.Point {
	return image.Pt(
		max(0, min(w-1, int(math.Round(x)))),
		max(0, min(h-1, int(math.Round(y)))),
	)
}
