...
func TestResizeNRGBAToGrayPairMatchesIndependentResults(t *testing.T) {
	...
	accented, raw := raster.ResizeNRGBAToGrayPair(src, 7, 5, 20)
	wantAccented := raster.ResizeNRGBAToGray(src, 7, 5, 20)
	wantRaw := raster.ResizeNRGBAToGray(src, 7, 5, 0)
...
func TestCloneImage_DeepCopy(t *testing.T) {
	...
	c := dst.NRGBAAt(1, 1)
	if c.R != 255 || c.G != 0 {
		t.Fatal("raster.CloneNRGBA did not make a deep copy")
	}
}
...
func TestSubImage_CorrectExtract(t *testing.T) {
	...
	sub := raster.CropNRGBA(src, image.Rect(3, 3, 8, 8))
...
func TestSubImage_ClampsToBounds(t *testing.T) {
	...
	sub := raster.CropNRGBA(src, image.Rect(-10, -10, 100, 100))
...
func TestGoodFeaturesToTrack_BlackImage(t *testing.T) {
	...
	pts, _ := cornerdetect.Detect(context.Background(), gray, cornerdetect.Options{MaxCorners: 10, QualityLevel: 0.01, MinDistance: 10, BlockSize: 3})
	if len(pts) != 0 {
...
func TestGoodFeaturesToTrack_SingleCorner(t *testing.T) {
	...
	pts, _ := cornerdetect.Detect(context.Background(), gray, cornerdetect.Options{MaxCorners: 20, QualityLevel: 0.01, MinDistance: 5, BlockSize: 3})
	if len(pts) == 0 {
...
func TestGoodFeaturesToTrack_RespectsMaxCorners(t *testing.T) {
	...
	pts, _ := cornerdetect.Detect(context.Background(), gray, cornerdetect.Options{MaxCorners: 2, QualityLevel: 0.01, MinDistance: 5, BlockSize: 3})
	if len(pts) > 2 {
...
func TestApplyCircularMask_CenterPixel(t *testing.T) {
	...
	dst := ApplyCircularMask(src, center, 20, 5, 0, bg)
...
func TestApplyCircularMask_Dimensions(t *testing.T) {
	...
	bg := color.NRGBA{A: 255}
	dst := ApplyCircularMask(src, image.Pt(15, 20), 10, 5, 0, bg)
...
func TestResizeGray_Dimensions(t *testing.T) {
	...
	dst := cornerdetect.ResizeGray(src, 50, 40)
...
func TestResizeGray_Upscale(t *testing.T) {
	...
	dst := cornerdetect.ResizeGray(src, 4, 4)
...
func TestResizeGray_ZeroSize(t *testing.T) {
	...
	dst := cornerdetect.ResizeGray(src, 0, 0)
...
func TestResizeGray_PreservesContent(t *testing.T) {
	...
	dst := cornerdetect.ResizeGray(src, 2, 2)
	for y := 0; y < 2; y++ {
		for x := 0; x < 2; x++ {
...
func TestResizeNRGBA_Dimensions(t *testing.T) {
	...
	dst := raster.ResizeNRGBA(src, 50, 100)
...
func TestResizeNRGBA_ZeroSize(t *testing.T) {
	src := image.NewNRGBA(image.Rect(0, 0, 10, 10))
	dst := raster.ResizeNRGBA(src, 0, 0)
	if dst.Bounds().Dx() != 10 {
		t.Fatal("zero-size resize should return original")
	}
}
...
func TestResizeNRGBA_PreservesColor(t *testing.T) {
	...
	dst := raster.ResizeNRGBA(src, 2, 2)
	for y := 0; y < 2; y++ {
		for x := 0; x < 2; x++ {
			c := dst.NRGBAAt(x, y)
