// Package compositor implements planar image stitching.
package compositor

// This file contains the standalone planar image stitching engine.
//
...
//  2. Detect Shi-Tomasi corners through internal/cornerdetect.
...
// Stitch assembles a sequence of images that share overlap between
...
func Stitch(imgs []*image.NRGBA) (*image.NRGBA, error) {
	n := len(imgs)
	if n < 2 {
...
func stitchDetectAndDescribe(img *image.NRGBA) ([]stitchFeature, float64) {
	...
	if mx := max(w, h); mx > compositorWorkDim {
		...
		workImg = raster.ResizeNRGBA(img, nw, nh)
	}

	gray := raster.ToGrayscale(workImg)
	// Use a moderate quality level and minimum distance to get a good spread.
	pts, _ := cornerdetect.Detect(context.Background(), gray, cornerdetect.Options{MaxCorners: compositorMaxFeats, QualityLevel: 0.01, MinDistance: 10, BlockSize: 7})
				...
				for ox := 0; ox < outW; ox++ {
					...
					if wSum > 0 {
						out.SetNRGBA(ox, oy, color.NRGBA{
							R: raster.ClampByte(int(rSum/wSum + 0.5)),
							G: raster.ClampByte(int(gSum/wSum + 0.5)),
							B: raster.ClampByte(int(bSum/wSum + 0.5)),
							A: 255,
						})
					}
				}
