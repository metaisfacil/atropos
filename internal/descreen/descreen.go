// Package descreen implements FFT-based screening-pattern removal.
package descreen
	...
	pFor(origRows, workers, func(sy, ey int) {
		...
		for y := sy; y < ey; y++ {
			...
			for x := 0; x < origCols; x++ {
				...
				dst.Pix[off] = raster.ClampByte(int(value + 0.5))
			}
		}
	})
}
	...
	pFor(origRows, workers, func(sy, ey int) {
		...
		for y := sy; y < ey; y++ {
			...
			for x := 0; x < origCols; x++ {
				...
				dst.Pix[dstOff] = raster.ClampByte(int(math.Round(origR + delta)))
				dst.Pix[dstOff+1] = raster.ClampByte(int(math.Round(origG + delta)))
				dst.Pix[dstOff+2] = raster.ClampByte(int(math.Round(origB + delta)))
...
// Apply is the production pure-Go descreen path. It uses planned
...
func Apply(src *image.NRGBA, thresh, radius, middle, highlightRestore int, logf func(string, ...interface{})) *image.NRGBA {
	totalStart := time.Now()
	b := src.Bounds()
	origRows, origCols := b.Dy(), b.Dx()
				...
				if blendStrength > 0 {
					...
					if hf > 0 {
						...
						for channel := 0; channel < 3; channel++ {
							...
							dst.Pix[dstOff+channel] = raster.ClampByte(int(filtered*(1-blend) + original*blend + 0.5))
						}
					}
				}
				dst.Pix[dstOff+3] = src.Pix[srcOff+3]
...
// ApplyLuminance is the fast descreen path. It detects and suppresses
...
func ApplyLuminance(src *image.NRGBA, thresh, radius, middle, highlightRestore int, logf func(string, ...interface{})) *image.NRGBA {
	totalStart := time.Now()
	b := src.Bounds()
	origRows, origCols := b.Dy(), b.Dx()
	paddedRows := nextSmoothFFT(origRows)
