	...
	pFor(origRows, nCPU, func(sy, ey int) {
		for y := sy; y < ey; y++ {
			for x := 0; x < origCols; x++ {
				...
				dr := raster.ClampByte(int(results[0][y*origCols+x] + 0.5))
				dg := raster.ClampByte(int(results[1][y*origCols+x] + 0.5))
				db := raster.ClampByte(int(results[2][y*origCols+x] + 0.5))
				if blendStrength > 0 {
					...
					if hf > 0 {
						...
						dr = raster.ClampByte(int(float64(dr)*(1-blend) + origR*blend + 0.5))
						dg = raster.ClampByte(int(float64(dg)*(1-blend) + origG*blend + 0.5))
						db = raster.ClampByte(int(float64(db)*(1-blend) + origB*blend + 0.5))
					}
				}
				dst.Pix[dstOff] = dr
				dst.Pix[dstOff+1] = dg
