...
// Apply reproduces the software dust-removal path: luminance Sobel
// seeds, square dilation, component shape and area filtering, enclosed-hole
// filling, and a local median repair.
func Apply(src *image.NRGBA, level string, dpi float64) (*image.NRGBA, int, float64, error) {
	...
	if dpi > profile.baselineDPI {
		...
		detectImage = raster.ResizeNRGBA(src, newW, newH)
	}
		...
		for x := 0; x < width; x++ {
			...
			for len(stack) > 0 {
				...
				for _, adjacentY := range [...]int{span.y - 1, span.y + 1} {
					...
					searchX := max(0, span.x0-1)
					searchEnd := min(width-1, span.x1+1)
					for searchX <= searchEnd {
						...
						minX = min(minX, run.x0)
						maxX = max(maxX, run.x1)
						minY = min(minY, run.y)
						maxY = max(maxY, run.y)
						searchX = run.x1 + 1
					}
	...
	for y := 0; y < h; y++ {
		for x := 0; x < w; x++ {
			...
			for _, offset := range repairOffsets {
				...
				n := 0
				for sampleY := max(0, targetY-2); sampleY <= min(h-1, targetY+2); sampleY++ {
					for sampleX := max(0, targetX-2); sampleX <= min(w-1, targetX+2); sampleX++ {
